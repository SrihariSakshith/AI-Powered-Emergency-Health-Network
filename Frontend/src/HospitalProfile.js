...
const HospitalProfile = ({ username }) => {
  ...
  useEffect(() => {
    const fetchHospitalData = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/hospital-profile/api/hospital/${username}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3001/hospital-profile/api/hospital/${username}`, updatedData);
  ...
  const handleDonationDelete = async () => {
    try {
      await axios.delete(`http://localhost:3001/hospital-profile/api/hospital/${username}/donation`);
