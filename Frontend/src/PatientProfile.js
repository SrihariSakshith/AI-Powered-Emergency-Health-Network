...
const PatientProfile = ({ username }) => {
    ...
    useEffect(() => {
        const fetchPatientData = async () => {
            try {
                const response = await axios.get(`http://localhost:3001/patient-profile/api/patient/${username}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:3001/patient-profile/api/patient/${username}`, {
                report: formData,
                donation,
            });
    ...
    const handleRemoveDonation = async () => {
        try {
            await axios.delete(`http://localhost:3001/patient-profile/api/patient/${username}/donation`);
            setHasDonated(false);
            setDonation('');
