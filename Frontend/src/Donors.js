    ...
    const fetchDonorsData = async () => {
      try {
        ...
        const donorsResponse = await axios.get(
          `${process.env.REACT_APP_API_BASE_URL || 'http://localhost:3001'}/donors/api/donors`
        );
        ...
        try {
          const locationResponse = await axios.get(
            `${process.env.REACT_APP_API_BASE_URL || 'http://localhost:3001'}/donors/api/user-location`,
        ...
        if (role === 'patient') {
          try {
            const bloodGroupResponse = await axios.get(
              `${process.env.REACT_APP_API_BASE_URL || 'http://localhost:3001'}/donors/api/user-blood-group`,
              { params: { username } }
            );
